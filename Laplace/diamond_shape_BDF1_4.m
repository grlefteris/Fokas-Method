...
[Xs Ys k hss xs ys hxs hys ws N]=domain(n1,Np,1,0);
[Xo Yo k hso xo yo hxo hyo wo N]=domain(n2,Np,1,0);
...
for i=1:k-1
    ...
    Q{i}=restrict(((n1/2)+(n2/2)),N{i});
    P{i}=kron(eye(((n1/2)+(n2/2))),Lbasis(Nl(i),N{i}));
    ...
    A{i}=PLHS_Dirichlet(((n1/2)+(n2/2)),Nl(i),M,R,m{i},h{i},1);
    Phat{i}=Ptransform(((n1/2)+(n2/2)),Nl(i),M,R,m{i},h{i});
...
K=sparse(MAT);
tic;Pc=colperm(K);K2=K(:,Pc);R2=qr(K2,0);xx2=R2\(R2'\(K2'*VEC));xx2(Pc)=xx2;toc
sol=xx2;
% 
ind=0;
XX=[];YY=[];EE=[];UU=[];
for i=1:k-1
    u{i}=real(sol(ind+1:ind+((n1/2)+(n2/2))*Nl(i)));
...
hold all
for i=1:k-1
    mesh(reshape(X{i},N{i},n),reshape(Y{i},N{i},n),reshape(P{i}*u{i},N{i},n));
end
subplot(1,3,2)
hold all
for i=1:k-1
