...
% %     mesh(XT,YT,abs(UT-UA)./norm(UA,inf));
%     mesh(XT,YT,UT);
% 
% %     plot3(XT,YT,UT,'k*')
%     
...
% hold all
% ind2=2*sum(sj);
% mesh(reshape(X{1},N{1},n),reshape(Y{1},N{1},n),abs(reshape(U{1},N{1},n)-reshape(P{1}*u{1},N{1},n))/norm(U{1},inf));
% for i=1:10%(hs/hs1)
%             
%     ut{i}=real(sol(ind2+1:ind2+n*N{k}));
%     
