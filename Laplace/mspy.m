...
if isempty(color), co = get(cax,'colororder'); color = 'k'; end
...
hold all
[i,j] = find(S);[ii,jj] = find(~S);
if isempty(i), i = NaN; j = NaN; end
if isempty(S), marker = 'none'; end
plot(j,i,'marker','*','markersize',10, ...
   'linestyle',linestyle,'color',color);

if isempty(ii), ii = NaN; jj = NaN; end
plot(jj,ii,'marker','o','markersize',3, ...
   'linestyle',linestyle,'color','k');
